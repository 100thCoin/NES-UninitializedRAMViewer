	...
	; This ROM will print $100 bytes on screen at a time, allowing you to see the uninitialized VRAM on your console.
	; NOTE: This will not work on an Everdrive N8 Pro, as that initializes RAM before booting the game.
	;			- It might work from power on with an Everdrive N8 pro though. The same cannot be said about the CPU RAM Viewer.
...
RESET:
	            ; Okay, here's the challenge. Copy VRAM into RAM, and then throughout the rest of this entire ROM, I am not allowed to use a single byte of CPU RAM, including the stack.
...
SetUpPaletteLoop:
	...
	LDY #$00
	LDA $2007    ; Prep the $2007 read buffer.
NT_Copy0:
	LDA $2007    ; Read from the nametable
	STA $0000,Y  ; Store it in RAM with offset Y
	INY
	BNE NT_Copy0 ; increment Y until Y=0.
NT_Copy1:
	LDA $2007    ; Read from the nametable
	STA $0100,Y  ; Store it in RAM with offset Y
	INY
	BNE NT_Copy1 ; increment Y until Y=0.
NT_Copy2:
	LDA $2007    ; Read from the nametable
	STA $0200,Y  ; Store it in RAM with offset Y
	INY
	BNE NT_Copy2 ; increment Y until Y=0.
NT_Copy3:
	LDA $2007    ; Read from the nametable
	STA $0300,Y  ; Store it in RAM with offset Y
	INY
	BNE NT_Copy3 ; increment Y until Y=0.
NT_Copy4:
	LDA $2007    ; Read from the nametable
	STA $0400,Y  ; Store it in RAM with offset Y
	INY
	BNE NT_Copy4 ; increment Y until Y=0.
NT_Copy5:
	LDA $2007    ; Read from the nametable
	STA $0500,Y  ; Store it in RAM with offset Y
	INY
	BNE NT_Copy5 ; increment Y until Y=0.
NT_Copy6:
	LDA $2007    ; Read from the nametable
	STA $0600,Y  ; Store it in RAM with offset Y
	INY
	BNE NT_Copy6 ; increment Y until Y=0.
NT_Copy7:
	LDA $2007    ; Read from the nametable
	STA $0700,Y  ; Store it in RAM with offset Y
	INY
	BNE NT_Copy7 ; increment Y until Y=0.
